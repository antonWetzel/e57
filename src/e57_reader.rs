...
impl E57Reader {
	...
	pub fn new(mut reader: File) -> Result<Self> {
		...
		let mut reader = PagedReader::new(reader, header.page_size).read_err("Failed creating paged CRC reader")?;
...
mod tests {
	...
	fn bunny_point_count() {
		...
		for file in files {
			...
			let pcs = reader.pointclouds();
			let pc = pcs.first().unwrap();
			let points: Vec<RawValues> = reader.pointcloud(pc).unwrap().map(|p| p.unwrap()).collect();
