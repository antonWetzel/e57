...
impl<T: Write + Read + Seek> E57Writer<T> {
	...
	pub fn new(writer: T, guid: &str) -> Result<Self> {
		...
		Ok(Self { writer, pointclouds: Vec::new(), root })
	}
	...
	pub fn add_pointcloud(&mut self, guid: &str, prototype: Vec<Record>) -> Result<PointCloudWriter<T>> {
...
mod tests {
	use super::*;
	use crate::{
		E57Reader, Point, Quaternion, RawValues, RecordDataType, RecordName, RecordValue, Transform, Translation,
	};
	...
	fn scaled_integers() {
		...
		{
			let mut writer = E57Writer::from_file(out_path, "file_guid").unwrap();
			const SCALED_INT: RecordDataType =
				RecordDataType::ScaledInteger { min: -1000, max: 1000, scale: 0.001 };
			let prototype = vec![
				Record {
	...
	fn write_read_meta_data() {
...

		{
			...
			pc_writer.set_transform(Some(Transform {
				rotation:    Quaternion { w: 1.1, x: 2.2, y: 3.3, z: 4.4 },
				translation: Translation { x: 5.5, y: 6.6, z: 7.7 },
			}));
			pc_writer.finalize().unwrap();
			e57_writer.finalize().unwrap();
		}
