...
impl IndexPacketHeader {
	pub fn read<T: std::io::Read + std::io::Seek>(reader: &mut PagedReader<T>) -> Result<Self> {
		...
		// Parse values
		let packet_length = u16::from_le_bytes(buffer[1..3].try_into().internal_err(WRONG_OFFSET)?) as u64 + 1;
		...
		Ok(Self { packet_length, entry_count, index_level })
	}
}
...
impl DataPacketHeader {
	...
	pub fn read<T: std::io::Read + std::io::Seek>(reader: &mut PagedReader<T>) -> Result<Self> {
		...
		let packet_length = u16::from_le_bytes(buffer[1..3].try_into().internal_err(WRONG_OFFSET)?) as u64 + 1;
		let bytestream_count = u16::from_le_bytes(buffer[3..5].try_into().internal_err(WRONG_OFFSET)?);
