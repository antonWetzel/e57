	...
	fn read_page(&mut self, page: u64) -> Result<()> {
		...
		if expected_checksum != calculated_checksum {
			...
			Err(Error::new(
				ErrorKind::InvalidData,
				format!(
					"Detected invalid checksum (expected: {expected_checksum:?}, actual: {calculated_checksum:?}) for \
					 page {page}"
				),
			))
		} else {
...
impl<T: Read + Seek> Read for PagedReader<T> {
	fn read(&mut self, buf: &mut [u8]) -> Result<usize> {
		...
		buf[..read_size].copy_from_slice(&self.page_buffer[page_offset as usize..page_offset as usize + read_size]);
