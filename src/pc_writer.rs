...
impl<'a, T: Read + Write + Seek> PointCloudWriter<'a, T> {
	pub(crate) fn new(
		...
		let has_index = prototype.iter().any(|p| {
			p.name == RecordName::ReturnIndex || p.name == RecordName::ColumnIndex || p.name == RecordName::RowIndex
		});
	...
	pub fn add_point(&mut self, values: RawValues) -> Result<()> {
		...
		for (i, p) in self.prototype.iter().enumerate() {
			...
			if p.name == RecordName::CartesianX || p.name == RecordName::CartesianY || p.name == RecordName::CartesianZ
			{
			...
			if p.name == RecordName::RowIndex || p.name == RecordName::ColumnIndex || p.name == RecordName::ReturnIndex
			{
