...
impl Error {
	...
	{
		Err(Error::Invalid { desc: desc.to_string(), source: None })
	}
	...
	where
		C: Display + Send + Sync + 'static,
	{
		Err(Error::NotImplemented { desc: desc.to_string() })
	}
	...
	where
		C: Display + Send + Sync + 'static,
	{
		Err(Error::Internal { desc: desc.to_string(), source: None })
	}
}
...
impl<T> Converter<T, Infallible> for Option<T> {
		...
		C: Display + Send + Sync + 'static,
	{
		match self {
			Some(ok) => Ok(ok),
			None => Err(Error::Read { desc: desc.to_string(), source: None }),
		}
	}
		...
		C: Display + Send + Sync + 'static,
	{
		match self {
			Some(ok) => Ok(ok),
			None => Err(Error::Invalid { desc: desc.to_string(), source: None }),
		}
	}
		...
		C: Display + Send + Sync + 'static,
	{
		match self {
			Some(ok) => Ok(ok),
			None => Err(Error::Internal { desc: desc.to_string(), source: None }),
		}
	}
		...
		C: Display + Send + Sync + 'static,
	{
		match self {
			Some(ok) => Ok(ok),
			None => Err(Error::Write { desc: desc.to_string(), source: None }),
		}
	}
}
